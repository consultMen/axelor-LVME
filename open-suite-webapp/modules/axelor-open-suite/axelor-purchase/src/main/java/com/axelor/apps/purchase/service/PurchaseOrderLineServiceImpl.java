...
public class PurchaseOrderLineServiceImpl implements PurchaseOrderLineService {
  ...
  private BigDecimal getPackagingCoef(PurchaseOrderLine line) {

    // coef = poidsParColis / nbUnitesParColis (kg/unité)
    if (line.getPoidsParColis() != null
        && line.getNbUnitesParColis() != null
        && line.getNbUnitesParColis().signum() > 0) {

      return line.getPoidsParColis()
          .divide(
              line.getNbUnitesParColis(),
  ...
  @Override
  public Map<String, BigDecimal> compute(PurchaseOrderLine line, PurchaseOrder po)
      throws AxelorException {

    HashMap<String, BigDecimal> map = new HashMap<>();
    if (po == null || line == null) return map;

    // Prix/kg obligatoire (ou prix TTC/kg si inAti)
    if ((!po.getInAti() && line.getPrice() == null)
        || (po.getInAti() && line.getInTaxPrice() == null)) {
      return map;
    }

    // unitésTot obligatoire pour ton calcul
    if (line.getUnitesTot() == null) {
      return map;
    }

    BigDecimal taxRate = BigDecimal.ZERO;
    if (CollectionUtils.isNotEmpty(line.getTaxLineSet())) {
      taxRate = taxService.getTotalTaxRate(line.getTaxLineSet());
    }

    BigDecimal coef = getPackagingCoef(line);

    // prixKgDiscounted = prix/kg après remise
    BigDecimal prixKgDiscounted = computeDiscount(line, po.getInAti());

    // paUnite = prix/kg × coef
    BigDecimal paUnite =
        prixKgDiscounted
            .multiply(coef)
    ...
    if (!po.getInAti()) {
      // Montant HT = unitésTot × paUnite
      exTaxTotal = computeAmount(line.getUnitesTot(), paUnite, currencyScaleService.getScale(po));

      inTaxTotal =
          currencyScaleService.getScaledValue(po, exTaxTotal.add(exTaxTotal.multiply(taxRate)));
    } else {
      // Ici paUnite est TTC/unité, donc TTC total = unitésTot × paUnite
      inTaxTotal = computeAmount(line.getUnitesTot(), paUnite, currencyScaleService.getScale(po));

      exTaxTotal =
          inTaxTotal.divide(
              taxRate.add(BigDecimal.ONE), currencyScaleService.getScale(po), RoundingMode.HALF_UP);
    }

    BigDecimal companyExTaxTotal = getCompanyExTaxTotal(exTaxTotal, po);
    BigDecimal companyInTaxTotal =
        currencyScaleService.getCompanyScaledValue(
            po, companyExTaxTotal.add(companyExTaxTotal.multiply(taxRate)));
    ...
    line.setPriceDiscounted(prixKgDiscounted);

    if (line.getProduct() != null) {
      line.setMaxPurchasePrice(getPurchaseMaxPrice(po, line));
      map.put("maxPurchasePrice", line.getMaxPurchasePrice());
    }
    ...
    map.put("priceDiscounted", prixKgDiscounted);
